...
def _serialize_tensor_value(
  ...
  # `np.object_` instead.
  # Note: This is required before calling `array.to_proto` because `value` can
  # be types other than `array.Array`.
  if type_spec.dtype.type is np.str_:
  ...
  if not array_shape.is_compatible_with(value.shape, type_spec.shape):
    raise TypeError(
        f'Cannot serialize array with shape {value.shape} to shape'
        f' {type_spec.shape}.'
    )
